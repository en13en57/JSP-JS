...
SELECT EMP_ID, EMP_NAME  FROM TEMP
UNION 
SELECT EMP_ID, BONUS_RATE  FROM TCOM; -- 에러! 앞의 결과는 숫자, 문자 // 뒤의결과는 숫자, 숫자

SELECT EMP_ID FROM TEMP
UNION
SELECT EMP_ID, BONUS_RATE  FROM TCOM; -- 에러! 앞의 결과는 1개 // 뒤의 결과는 2개

-- UNION 사용 예
-- TEMP와 TCOM을 이용하여 사번, 연봉 , 커미션을 보여주는 SQL을 만들어보자
-- 이때 연봉과 커미션이 모두 존재하는 사번은 두줄로 나와야 한다.
SELECT EMP_ID , SALARY FROM TEMP ;
SELECT EMP_ID, COMM  FROM TCOM;

SELECT EMP_ID , SALARY FROM TEMP
UNION
SELECT EMP_ID, COMM FROM TCOM; -- 잘못된 결과 왜? 연봉자리에 커미션이 나온다.
...
SELECT EMP_ID FROM TEMP
MINUS
SELECT EMP_ID FROM TCOM; -- 12개 : 커미션을 받지 않는 직원의 사번 (차집합)
...
-- =================================================================================
-- 1. CARTESIAN JOIN : 조건없이 조인하는 방법 : n * m 개의 결과
-- =================================================================================

SELECT
	count(*)
FROM 
	TEMP e, TCOM c ; -- 160 : tcom(8) * temp(20)
	
-- 사용 예
-- temp 자료중에 과장을 읽어오는데 실제 DB내용이 2번 중복됙게 조회하시오
...
-- =================================================================================
-- 사번, 이름, 부서, 번호, 부서이름을 조회하시오!

SELECT EMP_ID , EMP_NAME , DEPT_CODE FROM TEMP t ;
SELECT DEPT_CODE , DEPT_NAME FROM TDEPT t ;

SELECT 
	EMP_ID , EMP_NAME , e.DEPT_CODE , d.DEPT_NAME 
FROM 
	TEMP e , TDEPT d
WHERE e.DEPT_CODE = d.DEPT_CODE ;

-- TCOM의 WORK_YEAR가 2001인 자료를 TEMP의 사번과 연결하여 COMM을 받는 직원의 이름, 연봉+커미션을 조회하시오
SELECT 
...
WHERE 
	e.EMP_ID  = c.EMP_ID 
AND c.WORK_YEAR = '2001';


-- =================================================================================
-- 3. NON EQUI JOIN : 조건에 부등호가 사용되어 조인하는 방법
-- =================================================================================
-- TEMP와 TCOM을 연결하여 과장 직급을 가질만한 나이의 사원 목록을 출력하시오
SELECT * FROM TEMP t ;

SELECT * FROM EMP_LEVEL el ;
UPDATE TEMP SET BIRTH_DATE  = ADD_MONTHS(BIRTH_DATE, 20*12); -- 생년월일을 20년 후로 모두 변경
...
-- TEMP와 TCOM을 연결하여 과장 직급의 연봉을 받는 사원 목록을 출력하시오
SELECT 
	l.LEV, '('||l.FROM_SAL ||'~'||l.TO_SAL||')', e.EMP_ID , e.EMP_NAME ,e.SALARY, e.LEV 
FROM 
	TEMP e, EMP_LEVEL l
WHERE 
...
-- =================================================================================
-- 3. OUTER JOIN : 일치하는 행이 없더라도 나오게 하는 
-- =================================================================================
...
SELECT 
   E.EMP_ID, C.EMP_ID ,E.SALARY ,C.COMM , NVL(C.COMM,0) 
FROM 
   TEMP E, TCOM C
WHERE 
   E.EMP_ID  = C.EMP_ID;
...
-- 이때, 수습 사원은 연봉 상한과 하한이 없더라도 나와야한다.
SELECT 
	E.EMP_ID , E.EMP_NAME , E.SALARY , L.FROM_SAL , L.TO_SAL 
FROM 
	TEMP E, EMP_LEVEL L	
WHERE 
	E.LEV = L.LEV ;
...
SELECT 
	E.EMP_ID , E.EMP_NAME ,L.LEV ,E.SALARY , NVL(L.FROM_SAL||'', ' ') 연봉하한, NVL(L.TO_SAL||'',' ') 연봉상한
FROM 
	TEMP E, EMP_LEVEL L	
WHERE 
	E.LEV = L.LEV(+) ;
...
SELECT 
	E.EMP_ID , E.EMP_NAME ,NVL(L.LEV||'','수습사원') ,E.SALARY , NVL(L.FROM_SAL||'', ' ') 연봉하한, NVL(L.TO_SAL||'',' ') 연봉상한
FROM 
...
-- =================================================================================
-- 4. SELF JOIN : 동일한 테이블을 여러번 사용하여 조인하는 방법 (반드시 별칭 사용) 
-- =================================================================================
-- 사용 예 
-- TDEPT 테이블을 이용하여 부서코드, 부서명, 상위부서코드 , 상위부서명을 조회하시오
SELECT * FROM TDEPT;
...
SELECT t1.DEPT_CODE 부서코드, t1.DEPT_NAME 부서명, t2.DEPT_CODE 상위부서코드, t2.DEPT_NAME 상위부서명 FROM TDEPT t1, TDEPT t2 WHERE t2.DEPT_CODE = t1.PARENT_DEPT ORDER BY t1.DEPT_CODE ;

-- 문제 : hr계정에서 사번, 이름, 메니저이름을 조회하시오
SELECT * FROM temp;

-- 종합문제 : 사번, 이름, 생일, 나한라 보다 생일이 빠른 사람 수를 읽어오시오
...
FROM 
	TEMP a, TEMP b
WHERE
	a.BIRTH_DATE < b.BIRTH_DATE (+) -- 조건이 늙은 < 젊은
GROUP BY 
