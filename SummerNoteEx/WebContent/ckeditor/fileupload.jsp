...
<%
	...
	String saveName = "";
	// 이미지 업로드
	// 1) 서버에 저장할 경로가 없으면 경로를 만들어 준다. 1
	File file = new File(application.getRealPath("/ckeditor/"));
	if(!file.exists()){
		file.mkdirs(); // make director
	}
	String realPath = application.getRealPath("/ckeditor/"); // 서버 절대 경로 2
	// 2) 이미지를 업로드 한다.
	boolean isMultipart = ServletFileUpload.isMultipartContent(request); // 3
	if(isMultipart){
		DiskFileItemFactory factory = new DiskFileItemFactory(); // DiskFileItemFactory 객체 생성 4
		...
		if(items!=null && items.size()>0){ // 파일이 있다면 5
			for(FileItem item : items){ // 반복
				if(item!=null && item.getSize()>0){ // 파일의 크기가 0보다 크면 6
					String originalFileName = item.getName(); // 원본 이름

					if(originalFileName!=null){
					// IE의 경우 원본파일 이름에 경로가 들어간다. 경로가 있으면 경로를 지워줘야 한다.
					if(originalFileName.contains(File.separator)){ // 경로 구분자가 있다면 8 
					...
					// 겹치지않는 ID를 만들어준다. -- 저장파일명으로 쓰자!! 7
					 String saveFileName = UUID.randomUUID().toString() + "_" + originalFileName;
				     item.write(new File(realPath + File.separator + saveFileName));

					// (맨 마지막) 리턴할 데이터를 만들어 주어야 한다
					 // {"uploaded" : 1, "fileName" : "test.jpg", "url" : "/img/test.jpg"}
					JsonObject json = new JsonObject();
					json.addProperty("uploaded", 1);
					json.addProperty("fileName", originalFileName);
					json.addProperty("url", request.getContextPath()+"/ckeditor/" + saveFileName);
					out.print(json);
					}
				}
			}
