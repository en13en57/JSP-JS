...
--
-- 호스트: localhost
-- 처리한 시간: 09-12-23 08:javadbjavadb35 
-- 서버 버전: 5.1.37
...
CREATE TABLE IF NOT EXISTS `HR_EMPLOYEES` (
  ...
  `COMMISSION_PCT` double(22,0) DEFAULT NULL COMMENT 'Commission percentage of the employee; Only employees in sales department elgible for commission percentage',
...
INSERT INTO `HR_REGIONS` (`REGION_ID`, `REGION_NAME`) VALUES
(1, 'Europe'),
(2, 'Americas'),
(3, 'Asia'),
...
ALTER TABLE `HR_COUNTRIES`
  ADD CONSTRAINT `COUNTR_REF_FK` FOREIGN KEY (`REGION_ID`) REFERENCES `HR_REGIONS` (`REGION_ID`);

--
-- Constraints for table `HR_DEPARTMENTS`
--
ALTER TABLE `HR_DEPARTMENTS`
  ADD CONSTRAINT `DEPT_LOC_FK` FOREIGN KEY (`LOCATION_ID`) REFERENCES `HR_LOCATIONS` (`LOCATION_ID`),
  ADD CONSTRAINT `DEPT_MGR_FK` FOREIGN KEY (`MANAGER_ID`) REFERENCES `HR_EMPLOYEES` (`EMPLOYEE_ID`);

--
-- Constraints for table `HR_EMPLOYEES`
--
ALTER TABLE `HR_EMPLOYEES`
  ADD CONSTRAINT `EMP_DEPT_FK` FOREIGN KEY (`DEPARTMENT_ID`) REFERENCES `HR_DEPARTMENTS` (`DEPARTMENT_ID`),
  ADD CONSTRAINT `EMP_JOB_FK` FOREIGN KEY (`JOB_ID`) REFERENCES `HR_JOBS` (`JOB_ID`),
  ADD CONSTRAINT `EMP_MANAGER_FK` FOREIGN KEY (`MANAGER_ID`) REFERENCES `HR_EMPLOYEES` (`EMPLOYEE_ID`);

--
-- Constraints for table `HR_JOB_HISTORY`
--
ALTER TABLE `HR_JOB_HISTORY`
  ADD CONSTRAINT `JHIST_DEPT_FK` FOREIGN KEY (`DEPARTMENT_ID`) REFERENCES `HR_DEPARTMENTS` (`DEPARTMENT_ID`),
  ADD CONSTRAINT `JHIST_EMP_FK` FOREIGN KEY (`EMPLOYEE_ID`) REFERENCES `HR_EMPLOYEES` (`EMPLOYEE_ID`),
  ADD CONSTRAINT `JHIST_JOB_FK` FOREIGN KEY (`JOB_ID`) REFERENCES `HR_JOBS` (`JOB_ID`);
