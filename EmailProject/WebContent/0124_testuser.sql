...
SELECT EMP_NAME 이름, SALARY 연봉, round(SALARY/18, -2) 월급 FROM	temp; -- 600% 보너스, 자리수가 음수면 양의자리수이다.


-- 홀수달은 100%, 짝수달은 200% 지급하라
SELECT EMP_NAME 이름, SALARY 연봉, round(SALARY/18, -2) "홀수달 월급", round(SALARY/18,-2)*2 "짝수달 월급" FROM	temp; -- 600% 보너스, 자리수가 음수면 양의자리수이다.


-- 위의 결과에 홀수달은 교통비 10만원 추가 짝수달은 교통비 20만원 추가해서 조회하시오
SELECT EMP_NAME 이름, SALARY 연봉, round(SALARY/18, -2) + 100000 "홀수달 월급", round(SALARY/18,-2)*2 +200000 "짝수달 월급" FROM temp; -- 600% 보너스, 자리수가 음수면 양의자리수이다.

--temp테이블에서 취미가 null이 아닌 직원의 목록만 조회하시오
SELECT * FROM temp WHERE HOBBY !=NULL; -- 이놈은 안된다.
-- null의 비교는 is null 또는 is not null을 사용해야 한다.
SELECT * FROM temp WHERE HOBBY IS NOT NULL;
SELECT * FROM temp WHERE HOBBY IS NULL;

-- 취미가 없는 사원의 취미를 "음주"로 바꿔 모두 조회하시오
SELECT EMP_NAME 이름, HOBBY 취미 FROM temp;

--NVL함수 : NVL(변수또는 필드, NULL인 경우 가질 값)
SELECT EMP_NAME 이름, NVL(HOBBY,'음주') 취미 FROM TEMP ;

--NVL함수2 : NVL2(변수또는 필드, NULL이 아닌 경우 가질 값, NULL인 경우 가질 값)
SELECT EMP_NAME 이름, NVL2(HOBBY,HOBBY,'음주') 취미 FROM TEMP ;

-- TEMP의 자료 중 HOBBY의 값이 NULL인 사원을 '등산'으로 치환했을 때 HOBBY가 '등산'인 사람의 성명을 가져오는 문장을 작성하시오
SELECT EMP_NAME 이름, NVL(HOBBY,'등산') 취미 FROM temp WHERE NVL(HOBBY,'등산') = '등산';

-- 테이블에 별칭 사용
SELECT 
	EMP_ID 사번, EMP_NAME 이름, E.DEPT_CODE 부서코드, D.DEPT_CODE 부서이름
FROM 
	TEMP E, TDEPT D
WHERE 
...
-- 별칭을 사용하지 않을때
SELECT 
	EMP_ID 사번, EMP_NAME 이름, TEMP.DEPT_CODE 부서코드, TDEPT.DEPT_CODE 부서이름
FROM 
	TEMP, TDEPT
WHERE 
...
-- 그렇다면 어떻게 idx값으로 처리할까요?
-- sequence가 있다.

-- CREATE SEQUENCE 시퀀스이름(테이블명_seq_키필드명)
...
FROM 
	TEMP;
-- 그런데 괄호가 아니라 작은 따옴표로 직급을 나타내고 싶다면 어떻게 해야 할까요? -- 김길동'부장'

-- 작은 따옴표를 출력하고 싶은 경우는 '''' 를 써야한다
SELECT 
	EMP_NAME || '''' || LEV  || '''' 
FROM 
	TEMP ;
	
-- 사용자가 가진 객체들을 확인하고 싶은 경우는?
SELECT * FROM USER_OBJECTS;
SELECT OBJECT_name, OBJECT_type FROM USER_OBJECTS;

-- 오라클은 테이블을 작성하면 키필드를 기준으로 INDEX 자동생성 됩니다.
...
-- View를 만드려면 권한이 있어야 가능하다. sys 계정에서 권한을 부여해야 한다.
-- View를 만들 수 있는 권한 부여
...
SELECT 
	EMP_ID , SUBSTR(EMP_ID, 1, 4), SUBSTR(EMP_ID, 1, 3), SUBSTR(EMP_ID, 5, 2) 월
FROM 
	temp
WHERE
	SUBSTR(EMP_ID, 0, 4)='1997';
...
SELECT 
	EMP_ID 사번, EMP_NAME 이름, BIRTH_DATE, BIRTH_DATE || ''
FROM 
	TEMP 
WHERE 
	SUBSTR(BIRTH_DATE,4,2) = SUBSTR(SYSDATE, 4, 2); 
	
-- 근속년수가 30년 이상인 사원의 목록
-- EXTRACT 함수 : EXTRACT(찾는값 from 데이터)
SELECT 
	EXTRACT(YEAR FROM sysdate), EXTRACT(month FROM sysdate), EXTRACT(day FROM sysdate) FROM	dual;

SELECT 
	EMP_ID, EMP_ID /10000 입사연도 , ROUND(EMP_ID/10000) "입사년도(정수)", EXTRACT(YEAR FROM sysdate)-ROUND(EMP_ID/10000) 근속년수,
